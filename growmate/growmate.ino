// =============================================================
//  Growmate System
//  ESP8266 + Blynk + Web Dashboard
//  Features: EEPROM persistence, mDNS, OTA, non-blocking Blynk,
...
//  [0..1]  int   threshold
//  [2]     byte  presetCount
//  [3..N]  each preset: 13 bytes name + 2 bytes int thr = 15 bytes/slot
// -------------------------------------------------------------
...
// Moisture history — last 40 readings for chart
...
// =============================================================
//  HTML Dashboard  (light/dark mode via CSS variables + JS toggle)
// =============================================================
...
"--bd:#252525;--bd2:#2f2f2f;"
"--g:#4a7c59;--gh:#6aab7e;--gd:#2d4f38;"
"--am:#b8972a;--rd:#a03030;"
"--tx:#e8e8e8;--tm:#999999;--tl:#666666;"
"--thumb-border:#0a0a0a;"
"}"

// Light theme
"[data-theme=light]{"
"--bg:#f5f5f3;--s1:#ffffff;--s2:#f0f0ee;"
"--bd:#e0e0dc;--bd2:#d0d0cc;"
"--g:#3d6b4a;--gh:#3d6b4a;--gd:#c8dece;"
"--am:#8a6a10;--rd:#8b2020;"
"--tx:#1a1a1a;--tm:#555555;--tl:#888888;"
"--thumb-border:#f5f5f3;"
"}"
...
"<div class='hdr-left'>"
"<h1>Growmate</h1>"
"<p>Irrigation monitoring system</p>"
"</div>"
"<div class='hdr-right'>"
...
"<div class='mono'>ADC <span id='adcTxt' style='color:var(--tx)'>--</span> / 1024"
" &nbsp;&middot;&nbsp; <span id='pctTxt'>--%</span></div>"
"<div class='gw'>"
"<div class='gr'><span>0</span><span>512</span><span>1024</span></div>"
"<div class='gt'><div class='gf' id='gaugeFill' style='width:0%;background:var(--gh)'></div></div>"
"<div class='gz'>"
"<div class='zw'>Basah</div>"
"<div class='zn'>Normal</div>"
"<div class='zd'>Kering</div>"
"</div></div></div>"

// Pump
"<div class='card'>"
"<div class='lbl'>Pump</div>"
"<div style='margin-bottom:8px'><span class='led' id='pumpLed'></span>"
"<span class='ptxt' id='pumpTxt'>--</span></div>"
...
"</div>"
"<div class='sh'>"
"<span style='font-family:\"DM Sans\",sans-serif;font-size:10px;color:var(--tl);letter-spacing:1px'>Threshold</span>"
"<span id='thrVal'>--</span>"
"</div>"
"<input type='range' id='thrSlider' min='200' max='1024' value='700' step='1'"
...
"<input type='text' id='inName' placeholder='e.g. Selada' maxlength='12'>"
"</div>"
"<div class='field'>"
"<label>Threshold <span id='inThrVal'>600</span></label>"
"<input type='range' min='200' max='1024' value='600' step='1' id='inThr'"
" oninput=\"document.getElementById('inThrVal').innerText=this.value\">"
"</div>"
"</div>"
...
"<div class='xax'>"
"<span id='xL'>--</span><span id='xM'>--</span><span id='xR'>now</span>"
"</div>"
"</div>"
...
"document.getElementById('themeBtn').innerText=isDark?'Dark':'Light';"
...
// Presets
"const BUILTIN=["
"{n:'Cabai',t:650},{n:'Tomat',t:600},{n:'Kangkung',t:450},"
"{n:'Bayam',t:500},{n:'Padi',t:350},{n:'Jagung',t:700},"
"{n:'Bawang',t:750},{n:'Kentang',t:680},{n:'Kaktus',t:900},{n:'Stroberi',t:580}"
"];"
"let custom=[];"
"let selIdx=null;"
...
"const all=[...BUILTIN.map(p=>({...p,custom:false})),...custom.map(p=>({...p,custom:true}))];"
"all.forEach((p,i)=>{"
"const d=document.createElement('div');"
"d.className='pti'+(selIdx===i?' sel':'');"
"d.onclick=()=>selectPreset(i,p.t);"
"const del=p.custom?'<button class=\"ptd\" onclick=\"event.stopPropagation();delPreset('+i+')\">x</button>':'';"
"d.innerHTML=del+'<div class=\"ptn\">'+p.n+'</div><div class=\"ptv\">thr: '+p.t+'</div>"
"<div class=\"ptb\"><div class=\"ptbf\" style=\"width:'+Math.round(p.t/1024*100)+'%\"></div></div>';"
"g.appendChild(d);"
"});"
"const add=document.createElement('div');"
"add.className='addcard';add.onclick=openForm;"
"add.innerHTML='<div class=\"addicon\">+</div><span>Add Preset</span>';"
"g.appendChild(add);"
"}"

"function selectPreset(i,t){"
"selIdx=i;"
"document.getElementById('thrSlider').value=t;"
"document.getElementById('thrVal').innerText=t;"
"fetch('/api/threshold?val='+t);"
"renderPresets();"
"}"

"function delPreset(i){"
"const bl=BUILTIN.length;"
"if(i<bl)return;"
"custom.splice(i-bl,1);"
"if(selIdx===i)selIdx=null;"
...
"const t=parseInt(document.getElementById('inThr').value);"
"if(!n){document.getElementById('formErr').classList.add('show');return;}"
"custom.push({n,t});closeForm();renderPresets();saveCustom();"
"}"

"function saveCustom(){"
"fetch('/api/presets',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(custom)});"
"}"

"function onSlider(v){"
"document.getElementById('thrVal').innerText=v;"
"selIdx=null;renderPresets();"
"fetch('/api/threshold?val='+v);"
"}"
...
// Chart init — colors pulled from CSS vars at render time
"const ctx=document.getElementById('soilChart').getContext('2d');"
"Chart.defaults.font.family='DM Mono';"
"const chartData={labels:[],datasets:[{data:[],borderColor:'#4a7c59',"
"backgroundColor:'rgba(74,124,89,.08)',fill:true,tension:.4,borderWidth:1.5,pointRadius:0}]};"
"const chartOpts={responsive:true,maintainAspectRatio:false,animation:false,"
"plugins:{legend:{display:false}},"
"scales:{"
"y:{suggestedMin:0,suggestedMax:1024,grid:{color:'#1e1e1e'},border:{display:false},"
"ticks:{font:{family:'DM Mono',size:9},color:'#666'}},"
"x:{display:false}}};"
"const chart=new Chart(ctx,{type:'line',data:chartData,options:chartOpts});"

"function updateChartTheme(){"
"const light=document.documentElement.getAttribute('data-theme')==='light';"
"chart.options.scales.y.grid.color=light?'#e8e8e4':'#1e1e1e';"
"chart.options.scales.y.ticks.color=light?'#999':'#666';"
"chartData.datasets[0].borderColor=light?'#3d6b4a':'#4a7c59';"
"chartData.datasets[0].backgroundColor=light?'rgba(61,107,74,.08)':'rgba(74,124,89,.08)';"
"chart.update();"
"}"
...
"for(let i=0;i<10;i++){"
"const d=document.createElement('div');"
"d.className='dot';d.style.background=i<n?'var(--gh)':'var(--bd2)';w.appendChild(d);"
"}}"
...
"const el=document.getElementById('kondisiTxt');"
"el.innerText=d.kondisi;"
...
"const pt=document.getElementById('pumpTxt');"
"pt.innerText=d.pump;pt.className='ptxt '+(on?'cg':'cm');"
...
void handleApi() {
  ...
  // Store reading into history ring buffer
  ...
  String modeStr = (systemMode == 0) ? "AUTO"
                 : (systemMode == 1) ? "MANUAL ON"
                 :                     "MANUAL OFF";

  unsigned long elapsed = (lastWateredMillis == 0) ? 0
                        : (millis() - lastWateredMillis) / 1000;
...
void handleGetPresets() {
  ...
  for (int i = 0; i < customPresetCount; i++) {
    if (i > 0) json += ",";
    json += "{\"n\":\"" + String(customPresets[i].name) + "\","
          + "\"t\":"    + String(customPresets[i].thr)   + "}";
  }
  json += "]";
  server.send(200, "application/json", json);
}
...
void handlePostPresets() {
  ...
  savePresetsToEEPROM();
  Serial.println("Custom presets saved: " + String(customPresetCount));
  server.send(200, "text/plain", "OK");
}

// Returns last 5 moisture readings as JSON array for WA bot use
...
void handleWebAuto() { systemMode = 0; server.send(200, "text/plain", "OK"); }
...
void setup() {
  ...
  if (MDNS.begin("growmate")) {
    Serial.println("mDNS started — http://growmate.local");
  }

  ArduinoOTA.setHostname("ESP8266-Growmate");
